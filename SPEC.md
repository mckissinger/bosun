...
Out of scope: see the section above; in particular no change to mode values, no change to the rules headings, no renaming of the user's local directory or global instruction files, no behavior change anywhere.
Assumptions: the rename is token-level on tracked `.md`, `.json`, `.sh`, `.toml` files plus `git mv` of the skill and agent directories and files; historical evidence pointers in this spec are updated to the new paths since the mapping is one to one, with a note under the title; `gh repo rename` updates the local remote; the `fable-harness` plugin cache on this machine is left for the user to replace (done-condition 37).
Effort assumed: high (session default; not chosen at launch).
Checks: the grep in done-condition 31; `bash -n scripts/*.sh codex/scripts/*.sh`; JSON validation of the four manifests and `codex/hooks/hooks.json`; `/opt/homebrew/opt/python@3.12/bin/python3.12` `tomllib` parse of `codex/agents/*.toml`; temp-spec runs of both hooks; `CODEX_HOME=<tmp> codex plugin marketplace add "$PWD" && codex plugin list`; a real `bosun_scout` spawn from Astra in a temp project; `gh repo view`.
Branch: `bosun-rename`
