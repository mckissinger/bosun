...
Agents shipped (installed into `~/.codex/agents/` by `$fable-mode astra-crew` or `$fable-mode astra`, since plugins cannot bundle agents): `fable_scout` (gpt-5.6-luna, medium, read-only), `fable_verifier` (gpt-6-astra, high, read-only, the verifier agent text), and one worker per routing row, `fable_worker_small` (luna/max), `fable_worker_routine` (sol/medium), `fable_worker_feature` (sol/high), `fable_worker_hard` (sol/xhigh), all workspace-write with the worker instructions (no commit, no spec edits, run the checks, report). In `astra-crew` the brief's Execute step asks Astra to delegate the slice to the worker agent named by the task class and wait for it; in `astra` Astra implements itself.
...
22. `verified 2026-09-04 fable-verifier, rules/fable.md:23-32, skills/fable-brief/SKILL.md:17, skills/fable-mode/SKILL.md:22-34, README.md:88-97` Mode names. `rules/fable.md`, `skills/fable-brief/SKILL.md`, `skills/fable-mode/SKILL.md`, `skills/fable-verify/SKILL.md`, `README.md`, and this spec use `fable`, `fable-crew`, `astra-crew`, `astra`; `codex` is documented as an alias for `fable-crew` in the rules, the brief, and the mode skill; `/fable-mode` accepts the four names (and `codex`), preflights codex for `fable-crew`, and for `astra*` sets the line and says the project now runs in Codex; `/fable-brief` in an `astra*` mode stops and says so instead of running.
23. `verified 2026-09-04 fable-verifier, codex/.codex-plugin/plugin.json, .agents/plugins/marketplace.json, codex plugin list under temp CODEX_HOME` `codex/.codex-plugin/plugin.json` (name `fable-harness`, version `0.4.0`, `skills`, `hooks`, `interface`) and repo-root `.agents/plugins/marketplace.json` (plugin `fable-harness`, local source `./codex`) are valid JSON; with `CODEX_HOME` pointed at a temp dir, `codex plugin marketplace add "$PWD"` then `codex plugin list` shows the plugin.
24. `verified 2026-09-04 fable-verifier, codex/rules/astra.md:5-68, codex/hooks/hooks.json, six temp hook runs` `codex/rules/astra.md` carries the core rules for an Astra lead: effort ladder with the `ultra` ban, the spec contract including follow-ups, the four provider modes, the run policy, delegation to `fable_scout`, finishing through `$fable-verify`. `codex/hooks/hooks.json` declares a `SessionStart` command hook running `codex/scripts/session-start.sh`, which passes `bash -n`, emits `additionalContext` JSON, prints the rules unless `~/.codex/AGENTS.md` has the heading, the spec path with mode and run policy, and any checkpoint; a temp-spec run shows each.
25. `verified 2026-09-04 fable-verifier, codex/skills/*/SKILL.md; fable-brief:16-18,31,48,64-72,88; fable-mode:23-35` `codex/skills/fable-brief/SKILL.md`, `fable-verify`, `fable-checkpoint`, `fable-mode` exist with `name` and `description` frontmatter and port the Claude skills: Astra wording; subagent delegation in place of the Agent tool; `astra-crew` Execute delegates to the worker agent for the task class and waits, `astra` implements directly; run policy, staged briefs, re-validation, and exhausted-roadmap behavior carried over; `fable-mode` installs the agent files into `~/.codex/agents/` for the astra modes, sets a `fable*` line and says the project now runs in Claude Code (the mirror of the Claude side), and prints the routing table.
26. `verified 2026-09-04 fable-verifier, codex/agents/*.toml, tomllib parse, fields and values` `codex/agents/` holds `fable-scout.toml`, `fable-verifier.toml`, `fable-worker-small.toml`, `fable-worker-routine.toml`, `fable-worker-feature.toml`, `fable-worker-hard.toml`; each parses with `tomllib`, has `name` (underscored: `fable_scout`, `fable_verifier`, `fable_worker_small`, and so on, because Codex rejects hyphens in agent names), `description`, `developer_instructions`, `model`, `model_reasoning_effort`, `sandbox_mode`; scout and verifier are `read-only`; the four workers carry the routing table's model and effort and the worker instructions (no commit, no spec edits, run the checks, report files and checks).
27. `verified 2026-09-04 fable-verifier, README.md:20-29,55-59,90-95,116` `README.md` has a four-mode table, a "Codex install" section (marketplace add, plugin add, trust the hook, `$fable-mode astra-crew`), a description of how the astra modes run and what differs (native subagents, no usage file, desktop-app requirement of codex-cli 0.153.1+), and Codex rows in "What is in the plugin".
28. `verified 2026-09-04 fable-verifier, plugin.json:4, marketplace.json:22` `.claude-plugin/plugin.json` and `.claude-plugin/marketplace.json` bump to `0.4.0` and stay valid JSON.
...
Status: verified PASS WITH FOLLOW-UPS 2026-09-04; follow-ups in the section below and in the PR. Next: done-conditions 29 and 30 (human-check) in the ChatGPT desktop app once it carries codex-cli 0.153.1 or later.
...
- provider-modes, 2026-09-02: `skills/fable-brief/SKILL.md` "Find the provider mode" line understates which steps change (the "two lines" wording was fixed by the codex-port rename).
