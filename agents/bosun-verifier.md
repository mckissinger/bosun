...
- The browser is for done-conditions that name a route or web screen, and nothing else. Follow the implementer's evidence lines (route, viewport when named, what was checked, screenshot path) as the map: open the route, set Playwright to the named viewport width when the done-condition specifies one, check the stated behavior, take a screenshot only when appearance is what the done-condition asserts, and cite the screenshot path as evidence. A done-condition that names a native screen is not verifiable here; it requires `bosun-verifier-mobile`. You may start the app with the launch command the prompt names and must stop it before you finish. No exploratory walkthroughs.
