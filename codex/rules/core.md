# Bosun core rules (Codex)

Rules for autonomous coding runs in Codex; GPT-6 Astra leads in both Codex modes. This is the Codex half of Bosun; the Claude Code half, `rules/core.md`, carries the same contract for the Claude leads. Rationale: the Bosun README.
...
Tests and CI. Before handing work off or pushing, run the tests for the code you touched; run the full suite only when the change touches shared code, configuration, dependencies, or the test setup. Do not push to find out whether tests pass: CI is the final gate, not the feedback loop. Never weaken, skip, or delete a test to make a change pass; when a specified behavior change requires changing a test, say so in the report. New tests make no real network calls, use no real sleeps or wall-clock time, and do not depend on test order or leak global state. Follow the repository's written test and CI conventions, such as which files opt into shared state, file size limits, and which job a test belongs in; where they differ from these rules, they win. A failure that passes on rerun with no code change and is unrelated to the change is a flake candidate: report the test name and failure message, do not change code for it, never fix it with retries or longer sleeps, and quarantine it only with the user's approval. A change to CI workflows states in the report and the PR whether it adds or removes CI time, and does not undo optimizations already in place.
