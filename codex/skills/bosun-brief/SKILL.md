...
6. Execute. Work through the done-conditions and update each one's status in the spec as it lands. Follow the scope and edit rules in the core rules. If a done-condition names a route or screen, run the app and check it in the built-in browser before marking it done, and record an evidence line: the route, what was checked, and a screenshot path if one was taken. Do not end the turn to announce a next step; do the step. In astra-crew mode, replace this step with "Astra-crew mode" below.
...
6. In the report and in the spec's slice log, record: slice name, task class, route, first-verify verdict, and how many done-conditions named a route or screen against how many the verifier confirmed in its browser. There is no usage file; the app's usage view is the cost record.
