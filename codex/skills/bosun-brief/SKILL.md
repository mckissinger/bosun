...
2. Gather what the slice depends on: the files involved, the test setup, the exact check commands and any setup they need, and the branch state. Batch the reads. Hand side questions to the `bosun_scout` agent with a self-contained prompt and keep working while it runs.
...
6. Execute. Work through the done-conditions and update each one's status in the spec as it lands. Follow the scope, edit, and Tests and CI rules in the core rules. If a done-condition names a route or web screen, run the app and check it in the built-in browser before marking it done; a route-or-screen done-condition may name a viewport width, and its evidence line must then carry the viewport so the verifier can re-check it in Playwright at that width. Record the route, the viewport when named, what was checked, and a screenshot path if one was taken. If a done-condition names a native screen, set `Surface: ios`, check it before marking it done with the simulator tool this session has or with `xcrun simctl` from the shell when it has none, and record the evidence line as: screen, device, what was checked, screenshot path. Do not end the turn to announce a next step; do the step. In astra-crew mode, replace this step with "Astra-crew mode" below.
...
You brief, keep the spec, commit, and verify; a Sol or Luna worker subagent implements. The routing table and the `ultra` ban are in the core rules. If the worker agents are not installed (`~/.codex/agents/bosun-worker-*.toml`), run `$bosun-mode astra-crew` first.

Brief (step 3): add to the brief template

```
Task class: <small | routine | feature | hard, with the one-line reason>
Route: <bosun_worker_small | bosun_worker_routine | bosun_worker_feature | bosun_worker_hard>   (from the table, or the spec's own Route: override if the current-slice section has one)
```

Execute (step 6):

1. Write the worker prompt to `~/.codex/bosun/workers/<slug>/<slice>/prompt.md`, where `<slug>` is the checkpoint slug (cwd with the leading slash removed and every `/` or space replaced by `-`) and `<slice>` is a short name for this slice. The worker has no memory of this session, so the file must contain: the slice brief verbatim; the spec's decisions and out-of-scope lists; the "While working" rules from the core rules (scope, tests, targeted edits, and the route-or-screen browser check with its evidence line; the worker has Playwright MCP); the exact check commands, the app's launch command when a done-condition names a route or screen, and any setup they need; the worktree path; and these instructions: do not commit, do not edit the spec, run the checks before finishing, and end with a short report naming every file changed and every check run with its result.
...
3. When the worker finishes, read its report and `git diff` in the worktree. When the brief says `Surface: ios`, Astra checks the named native screen with the simulator tool this session has, or `xcrun simctl` from the shell when it has none, before marking it done. Update each done-condition's status in the spec from what the diff, the report, and that simulator check show, not from the report alone. If the worker committed anything, note it as a finding for the report and continue.
...
6. In the report and in the spec's slice log, record: slice name, task class, route, first-verify verdict, and how many done-conditions named a route or screen against how many the verifier confirmed in its browser or simulator. There is no usage file; the app's usage view is the cost record.
