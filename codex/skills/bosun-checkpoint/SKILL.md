...
6. **Next actions**: in the order to do them. If a staged brief exists under `~/.codex/bosun/workers/<slug>/`, name its path and whether it has been re-validated. If a worker subagent was mid-run, say so; its thread does not survive the session, so the next session re-delegates from the prompt file.
